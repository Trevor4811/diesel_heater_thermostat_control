...
int32_t BMP180_GetAltitude(float pressure_Pa, float temperature_C) {
    // Constants
    float P0 = 101325.0;  // Standard pressure at sea level in Pa
    float L = -0.0065;   // Standard temperature lapse rate in K/m
    float T0 = 288.15;   // Standard temperature at sea level in K
    float g = 9.81;     // Acceleration due to gravity in m/s²
    float M = 0.029;    // Molar mass of air in kg/mol
    float R = 8.314;    // Universal gas constant in J/(mol·K)
    float ft_to_m = 0.3048;  // Conversion factor for feet to meters

    // Convert temperature to Kelvin
    float temperature_K = temperature_C + 273.15;

    // Calculate altitude in meters
    float h_m = (T0 / L) * (1 - pow((pressure_Pa / P0), ((-R * L) / (g * M)))-1);

    // Convert altitude from meters to feet
    float h_ft= h_m * ft_to_m;
    return h_ft;
}
